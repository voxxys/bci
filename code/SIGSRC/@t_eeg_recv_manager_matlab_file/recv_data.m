...
nsamples_total = size(EEG.data.data,2);
...
data = zeros(size(EEG.data.data,1), nsamples_recv);
...
if pos_last < nsamples_total
    ...
    data = EEG.data.data(:, pos_first:pos_last);
...
%     mark_data(ev_lats_cur_rel) = ev_types_cur;
    ...
    for n = [1, 2]
        ...
        data(:, out_pos_first : out_pos_first+sz-1) = EEG.data.data(:, pos_first:pos_last);
