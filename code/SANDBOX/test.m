...
for i = 1:12
    ...
    [z_high,p_high,k_high] = butter(3, fmin/(Fs/2), 'high');
    [b_high,a_high] = zp2tf(z_high,p_high,k_high);

    [z_low,p_low,k_low] = butter(3, fmax/(Fs/2), 'low');
    [b_low,a_low] = zp2tf(z_low,p_low,k_low);

    % filter
    testdata{i} = filtfilt(b_high, a_high, testdata{i}')'; % data is original untouched data
    testdata{i} = filtfilt(b_low, a_low, testdata{i}')'; 
