...
for num_q = 1:numcomb
    ...
    if fmin > 0     
        [this.bandfilt_coeffs{inp_num}.b_high, this.bandfilt_coeffs{inp_num}.a_high] = butter(order_high, fmin/(fs/2), 'high');
        a_high{num_q} = this.bandfilt_coeffs{inp_num}.a_high;
        b_high{num_q} = this.bandfilt_coeffs{inp_num}.b_high;
    end

    if fmax < Inf  
        [this.bandfilt_coeffs{inp_num}.b_low, this.bandfilt_coeffs{inp_num}.a_low] = butter(order_low, fmax/(fs/2), 'low');    
        a_low{num_q} = this.bandfilt_coeffs{inp_num}.a_low;
        b_low{num_q} = this.bandfilt_coeffs{inp_num}.b_low;
    end
...
%              Zlast{inp_num,2} = Z;
            ...
            win_chunk = zeros(size(power_chunk));
        ...
        %  ADD CLASSIFICATION WITH PERCEPTRON HERE
